  ...
  return (
    <li className="flex py-3">
      <div className="h-21 w-21 flex-shrink-0 overflow-hidden rounded-md border border-gray-200">
        <div className='block h-full w-full cursor-pointer' onClick={handleImageClick}>
          <Image
            src={item.image}
            unoptimized
            alt={item.name}
      ...
      <div className="ml-4 flex flex-1 flex-col">
        ...
        <div className="flex flex-1 justify-between text-sm">
          <div className="flex items-center">
            ...
            <span className="min-w-3 text-center mx-3 font-bold text-[var(--color-text)]">{item.quantity}</span>
            <button
              onClick={() => updateQuantity(item.id, item.quantity + 1)}
              className="w-8 h-8 flex items-center justify-center bg-[var(--color-card-bg)] rounded-md text-[var(--color-badge)] font-bold shadow-md
                        cursor-pointer
