    ...
    >
      <div className="flex-1">
        <div className="relative w-full aspect-square bg-[var(--color-card-bg)]">
          <Image
            src={product.imagePath ? `${process.env.NEXT_PUBLIC_SUPABASE_STORAGE_MEDIUM_IMAGES}${product.imagePath}` : '/icons/file.svg'}
            unoptimized
            alt={product.name}
            fill
            priority={index < 2}
        ...
        <div className="pt-2 px-4">
          <h2 className="text-md font-normal text-[var(--color-text)] group-hover:text-[var(--color-navbar-bg)] text-left">{product.name}</h2>
        </div>
      </div>
